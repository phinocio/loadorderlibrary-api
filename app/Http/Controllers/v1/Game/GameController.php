...
final class GameController
{
    public function index(): AnonymousResourceCollection
    {
        $games = Cache::rememberForever(
            ...
            fn () => Game::query()->withCount(['lists' => fn (HasMany $query) => $query->where('is_private', false)])->orderBy('name')->get()
        );
    ...
    public function show(string $game): GameResource
    {
        $game = Cache::rememberForever(
            ...
            fn () => Game::query()->withCount(['lists' => fn (HasMany $query) => $query->where('is_private', false)])->where('slug', $game)->orWhere('name', $game)->firstOrFail()
        );
